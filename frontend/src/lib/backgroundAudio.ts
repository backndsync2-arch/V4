...
export class BackgroundAudioManager {
  ...
  private createAudioContext() {
    if (!this.audioContext) {
      ...
      // Don't resume here - will be resumed on first user interaction
    }
  ...
  private preventAudioSuspension() {
    ...
    const keepAlive = async () => {
      if (this.audioContext && 
        ...
        // Only try to resume if background mode is enabled
    ...
    // Check every 5 seconds (only if background is enabled)

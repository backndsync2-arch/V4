...
import { announcementsAPI, zonesAPI, schedulerAPI, musicAPI, adminAPI, authAPI, wsClient } from '@/lib/api';
import type { Announcement, Device, Schedule, MusicFile, Client } from '@/lib/types';
import {
  Play, Pause, SkipBack, SkipForward, Volume2, Shuffle, Repeat,
  Radio, AlertCircle, Wifi, WifiOff, Music, Plus, Edit2, Save, Clock
} from 'lucide-react';
...
export function DashboardEnhanced() {
  ...
  const { announcements, devices, schedules, zones, musicFiles, isLoading: loadingStates } = useFiles();
  const [musicQueue, setMusicQueue] = useState<MusicFile[]>([]);
  const [currentClient, setCurrentClient] = useState<Client | null>(null);

  const clientId = user?.role === 'admin' ? null : user?.clientId;

  // Set music queue from centralized state
  useEffect(() => {
    setMusicQueue(musicFiles);
  }, [musicFiles]);

  const isLoading = loadingStates.announcements || loadingStates.devices || loadingStates.schedules || loadingStates.zones || loadingStates.musicFiles;
  ...
  useEffect(() => {
    const loadClientData = async () => {
      ...
      try {
        ...
        const client = clientsList.find((c: any) => c.id === clientId);
        if (client) setCurrentClient(client as Client);
      } catch (error) {
        ...
        toast.error('Failed to load dashboard data', { description: error?.message || 'Please try again' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const handleDeviceStatusChange = (data: any) => {
      // console.log('Dashboard device status change:', data); // Debug logging removed
    ...
    const handleScheduleExecuted = (data: any) => {
      // console.log('Schedule executed:', data); // Debug logging removed
      ...
      schedulerAPI.getSchedules().then(schedulesData => {
    ...
    const handleSystemNotification = (data: any) => {
      // console.log('System notification:', data); // Debug logging removed
    ...
    const handleConnected = (data: any) => {
      if (data.type === 'events') {
        // console.log('Dashboard connected to global events WebSocket'); // Debug logging removed
        toast.success('Real-time updates active', {
          description: 'Connected to live system events'
        });
      }
    };
    ...
    const handleDisconnected = (data: any) => {
      if (data.type === 'events') {
        // console.log('Dashboard disconnected from global events WebSocket'); // Debug logging removed
        toast.warning('Real-time updates disconnected', {
          description: 'Attempting to reconnect...'
        });
        // Mark devices as potentially offline when disconnected
        setDevices(prev => prev.map(device => ({ ...device, status: 'offline' as const })));
      }
    };

    const handleError = (data: any) => {
      if (data.type === 'events') {
        console.error('Global events WebSocket error:', data.error);
  ...
  // Count unique floors from zones data
  const uniqueFloors = new Set(zones.map(zone => zone.floorId).filter(Boolean));
  const currentFloorCount = uniqueFloors.size || 1; // Default to 1 if no zones loaded yet
  ...
  const handleInstantPlay = () => {
    ...
    const targetZoneIds = Array.from(new Set(devices.map(d => d.zoneId).filter(Boolean))) as string[];
    playInstantAnnouncement(selectedAnnouncement, targetZoneIds, { duck: duckMusic });
  ...
  const handleSaveDuckingSettings = async () => {
    ...
    try {
      // Save ducking settings to backend
      const settingsData = {
        dashboard: {
          ducking: {
            enabled: duckMusic,
      ...
      // Use the auth API to update user settings
  ...
  const handleJumpToTrack = async (trackId: string) => {
    try {
      // TODO: Call API to jump to specific track in queue when endpoint is available
      const track = musicQueue.find(t => t.id === trackId);
      await new Promise(resolve => setTimeout(resolve, 500)); // Simulate API call
      toast.success(`Skipping to: ${track?.name || trackId}`);
    } catch (error: any) {
              ...
              <>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">{musicQueue.length} tracks</Badge>
                  </div>
                  ...
                  <SelectContent>
                    {availableAnnouncements.map(audio => (
                      <SelectItem key={audio.id} value={audio.id}>
                        {audio.title} ({formatDuration(audio.duration || 0)})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
